...
root_path = "/home/liuke/liuke/prj/score/CrystalTransformer/dataset/small_10/2018"
...
labels, labelsF = data[:,0].astype(int),data[:,1].astype(float)
...
optimizer = optim.Adam(CryT.parameters(), lr=2e-4, weight_decay=0.000001)
val = list()
tra = list()
#####################################cuda()#######################
CryT.float().cuda()
...
for epoch in range(100):
    ...
    if test_loss_F < best_MAE:  # 只要模型效果变好，就保存
        ...
        torch.save(CryT, '/home/liuke/liuke/prj/score/CrystalTransformer/ckpt2/best.pt')
