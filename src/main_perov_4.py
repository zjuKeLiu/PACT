...
root_path = "/home/liuke/liuke/prj/score/CrystalTransformer/dataset/perov"
...
labels, labelsF = data[:,0],data[:,4].astype(float)
...
optimizer = optim.Adam(CryT.parameters(), lr=LEARNING_RATE)#, weight_decay=0.0000001)
val = list()
tra = list()
#####################################cuda()#######################
CryT.float().cuda()
...
for epoch in range(200):
    ...
    if test_loss_F < best_MAE:  # 只要模型效果变好，就保存
        ...
        torch.save(CryT, '/home/liuke/liuke/prj/score/CrystalTransformer/multi_hierachy/test/4/best.pt')
